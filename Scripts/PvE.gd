...
onready var mover = $move_watcher
...
func generate_Move():
	var maxkiller = black_pawns[0]
	var maxkill = -1
	for pawn in black_pawns:
		var killcount = pawn.check_killable()
		if killcount.size() >= maxkill:
			maxkiller = pawn
			maxkill = killcount.size()
	var tokilled
	if maxkill > 0:
		var kills = maxkiller.check_killable()
		tokilled = kills[0]
		for kill in kills:
			if kill.points > tokilled.points:
				tokilled = kill
		maxkiller.index_on_board = tokilled.index_on_board
		kill_pawn(tokilled,true)
	if maxkill == 0:
		if soliders_black.size() > 0:
			var farthest = soliders_black[0]
			for sold in soliders_black:
				if sold.index_on_board.y > farthest.index_on_board.y and sold.check_moves().size() > 0:
					farthest = sold
			if farthest.firstmove == true:
				farthest.index_on_board.y += randi() % 2 + 1
			else:
				farthest.index_on_board.y += 1
		else:
			var pick = randi() % black_pawns.size()
			var moves = black_pawns[pick].check_moves()
			var randmove = randi() % moves.size()
			black_pawns[pick].index_on_board = moves[randmove]
		#DO SOMETHING IF ANY OF THE PAWN IS NOT KILLABLE
		#THIS WILL BE MOSTLY OACCURING IN STARTING OF THE GAME

	turn = 2 # THIS SHOULD BE THE LAST LINE
	...
	if black == true:
		...
		for i in range(white_pawns.size()):
			if white_pawns[i] == pawn:
				index = i
		for i in range(soliders_white.size()):
			if pawn == soliders_white[i]:
				solder_index = i
		if not solder_index == -1:
			soliders_white.remove(solder_index)
		...
		for i in range(black_pawns.size()):
			if black_pawns[i] == pawn:
				index = i
		for i in range(soliders_black.size()):
			if pawn == soliders_black[i]:
				solder_index = i
		if not solder_index == -1:
			soliders_black.remove(solder_index)
...
func _on_World_player_moved():
	generate_Move()


func _on_move_watcher_timeout():
	generate_Move()
