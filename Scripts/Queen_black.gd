...
onready var rays = [ray_down,ray_left,ray_up,ray_right,ray_down_left,ray_down_right,ray_up_left,ray_up_right]

func _ready():
	ray_down.add_exception(self)
	ray_down_left.add_exception(self)
	ray_down_right.add_exception(self)
	ray_up.add_exception(self)
	ray_up_left.add_exception(self)
	ray_up_right.add_exception(self)
...
func check_moves():
	...
	if ray_down_right.is_colliding():
		var dn = map.get_index_on_board(ray_down_right.get_collision_point())
		pos.append(dn)
	if ray_down_left.is_colliding():
		var dn = map.get_index_on_board(ray_down_left.get_collision_point())
		pos.append(dn)
	if ray_up_left.is_colliding():
		var dn = map.get_index_on_board(ray_up_left.get_collision_point())
		pos.append(dn)
	if ray_up_right.is_colliding():
		var dn = map.get_index_on_board(ray_up_right.get_collision_point())
		pos.append(dn)
...
func check_killable():
	...
	for ray in rays:
		if ray.is_colliding():
			if ray.get_collider().get_parent().is_in_group("white"):
